...
$(document).ready(function(){
	...
	if(${sportsGround.area=='1'}){
		$("#areaText").html("中区");
	}
	if(${sportsGround.area=='2'}){
		$("#areaText").html("西区");
	}
	if(${sportsGround.area=='3'}){
		$("#areaText").html("东区");
	}
	...
	<div id="content">
		<div >
  			...
  			<c:if test="${updateSuccess==null}">
  				<p class="text2">添加教学楼: <strong>${resourceName}</strong></p>
  			</c:if>
...
<div id="content" >
	<ul class="nav nav-tabs">
		<li  data-toggle="tab" ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/sportListPage?pageNum=1'">资源列表</a></li>
		<c:if test="${sportsGround.id==null}">
			<li class="active" data-toggle="tab" ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/addSportPage'">新增资源</a></li>
		</c:if>
		<c:if test="${sportsGround.id!=null}">
			<li class="active" data-toggle="tab" ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/addSportPage'">资源修改</a></li>
		</c:if>
	</ul>
	<div class="addForm">
		<form:form modelAttribute="sportsGround"  action="${APP_PATH}/manage/addSport" method="post">
			<form:hidden path="id"/>
		   ...
		   <div class="form-group">
		    	<label for="exampleInputEmail1" class="text1"> 资源名称</label></br>
				<form:input path="name" type="text" class="form-control"  placeholder="如:西区操场" style="width: 280px" />
		   </div>
		   <div class="form-group">
		    	<label for="exampleInputEmail1" class="text1"> 占地面积<strong> /m²</strong></label></br>
				<form:input path="space" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
				 type="text" class="form-control"  placeholder="如：3000" style="width: 280px" />
		   </div>
		   <div class="subBtn">
			   <c:if test="${sportsGround.id==null}">
