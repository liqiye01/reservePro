...
<body>
	<div id="content">
		<ul class="nav nav-tabs">
			<li class="active" ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/classroomListPage?pageNum=1'">资源列表</a></li>
			<li  ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/addClassroomPage'">新增资源</a></li>
		</ul>
		<div class="select">
			<form action="${APP_PATH}/manage/classroomListPage?pageNum=1">
		...
		<table class="table table-striped">
			...
			<c:forEach items="${pageInfo.list}" var="classroom">
				<tr>
					<td>
						<c:if test="${classroom.area=='1'}">中区</c:if>
						<c:if test="${classroom.area=='2'}">西区</c:if>
						<c:if test="${classroom.area=='3'}">东区</c:if>
					</td>
					<td>${classroom.buildingNum}</td>
					<td>${classroom.floor}</td>
					<td>${classroom.roomNum}</td>
					<td>${classroom.maxPopulation}</td>
					<td>
						<c:if test="${classroom.airConditioner=='1'}">
							是						
						</c:if>
						<c:if test="${classroom.airConditioner=='0'}">
							无						
						</c:if>
					</td>
					<td>
						<a href="${APP_PATH}/manage/updateClassroom?id=${classroom.id}">修改</a>
						<a onclick="return confirm('确认删除资源  【${classroom.buildingNum}教${classroom.roomNum}】吗？', this.href)"
							href="${APP_PATH}/manage/delClassroom?id=${classroom.id}&timeArrangeId=${classroom.timeArrangeId}
							&predictPersonId=${classroom.predictPersonId}&area=${area}&buildingNum=${buildingNum}
							&floor=${floor}&roomNum=${roomNum}&maxPopulation=${maxPopulation}
							&airConditioner=${airConditioner}&pageNum=${pageNum}&resourceName=${classroom.buildingNum}教${classroom.roomNum}">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<div class="paging">
		<nav aria-label="Page navigation">
		  <ul class="pagination">
		    <li id="previous">
		      <a href="${APP_PATH}/manage/classroomListPage?pageNum=${pre}" aria-label="Previous">
		        <span aria-hidden="true">&laquo;上一页</span>
		      </a>
		    </li>
		    <li id="next">
		      <a href="${APP_PATH}/manage/classroomListPage?pageNum=${nex}" aria-label="Next">
		...
		<div class="goForm">
			<form action="${APP_PATH}/manage/classroomListPage" >
