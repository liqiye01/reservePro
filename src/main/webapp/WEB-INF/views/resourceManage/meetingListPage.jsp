...
<body>
	<div id="content">
		<ul class="nav nav-tabs">
			<li class="active" ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/meetingListPage?pageNum=1'">资源列表</a></li>
			<li  ><a  onclick="javascript:window.location.href='${APP_PATH}/manage/addMeetingPage'">新增资源</a></li>
		</ul>
		<div class="select">
			<form action="${APP_PATH}/manage/meetingListPage?pageNum=1">
		...
		<table class="table table-striped">
			...
			<c:forEach items="${pageInfo.list}" var="meetingRoom">
				<tr>
					<td>
						<c:if test="${meetingRoom.area=='1'}">中区</c:if>
						<c:if test="${meetingRoom.area=='2'}">西区</c:if>
						<c:if test="${meetingRoom.area=='3'}">东区</c:if>
					</td>
					<td>${meetingRoom.buildingNum}</td>
					<td>${meetingRoom.floor}</td>
					<td>${meetingRoom.roomNum}</td>
					<td>${meetingRoom.maxPopulation}</td>
					<td>
						<a href="${APP_PATH}/manage/updateMeeting?id=${meetingRoom.id}">修改</a>
						<a onclick="return confirm('确认删除资源  【${meetingRoom.buildingNum}教${meetingRoom.roomNum}】吗？', this.href)"
							href="${APP_PATH}/manage/delMeeting?id=${meetingRoom.id}&timeArrangeId=${meetingRoom.timeArrangeId}
							&predictPersonId=${meetingRoom.predictPersonId}&area=${area}&buildingNum=${buildingNum}
							&floor=${floor}&roomNum=${roomNum}&maxPopulation=${maxPopulation}
							&pageNum=${pageNum}&resourceName=${meetingRoom.buildingNum}教${meetingRoom.roomNum}">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<div class="paging">
		<nav aria-label="Page navigation">
		  <ul class="pagination">
		    <li id="previous">
		      <a href="${APP_PATH}/manage/meetingListPage?pageNum=${pre}" aria-label="Previous">
		        <span aria-hidden="true">&laquo;上一页</span>
		      </a>
		    </li>
		    <li id="next">
		      <a href="${APP_PATH}/manage/meetingListPage?pageNum=${nex}" aria-label="Next">
		...
		<div class="goForm">
			<form action="${APP_PATH}/manage/meetingListPage" >
